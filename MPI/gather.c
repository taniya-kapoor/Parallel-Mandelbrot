/*
 SU project Taniya -- question MPI Gather
 */
...
int main (int argc, char * * argv) {
    ...
    int locheight = height/nbProc; //local block height
    ...
    // creating an array of minimum y for each block
    ...
    Compute (& imloc, nb_iter, x_min, x_max, arr[nbProc-myRank-1], arr[nbProc-myRank]); // each block is computed by different processors and stored at imloc
    
    MPI_Gather(& imloc.pixels[0],width*locheight,MPI_CHAR,& im.pixels[0],width*locheight,MPI_CHAR,0,MPI_COMM_WORLD); // the pixels from each imloc is gathered into im in processor 0
