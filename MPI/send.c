/*
 SU project Taniya -- MPI_Send/Recv Routine
 */
...
int main (int argc, char * * argv) {
  ...
  int size, rank, locheight, local_size, lsize;
  ...
  for(int jj=0; jj<lsize; jj++) // each processor computes a line
    {
      ...
      if(rank!=0)
      {
        MPI_Send(&(imloc.pixels[0]), local_size + 1, MPI_CHAR, 0, tag, MPI_COMM_WORLD); // each processor (except 0) sends the computed line
      }

      if(rank==0)
      	{
	  for(int i=0; i<width; i++)
	    im.pixels[jj*size*local_size+i] = imloc.pixels[i];

      	  for(int r=1; r<size; r++)
      	    {
	      l = (jj*size+r)*local_size;
      	      MPI_Recv(&(im.pixels[l]), local_size+1, MPI_CHAR, r, tag, MPI_COMM_WORLD, &status); // processor 0 recieves and stores the line correctly
	    }
	}
    }
    endtime = MPI_Wtime();
